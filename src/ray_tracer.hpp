...
public:
	...
	static float GetHighestPoint(std::vector<glm::vec3> edges);

	// Calculations
	static float CalculateSingleKnifeEdge(  glm::vec3 start_position, glm::vec3 edge_position,
                                            glm::vec3 end_position, float frequency);
	static float CalculateDiffractionByV(float v) ;
	static float CalculateVOfEdge(  glm::vec3 start_position, glm::vec3 edge_position,
                                    glm::vec3 end_position, float frequency) ;
	static void CalculateCorrectionCosines( glm::vec3 start_position, std::vector<glm::vec3> edges,
                                            glm::vec3 end_position, std::pair<float, float> & calculated_cosines) ;
	static void CalculateDirectPath(const Record & record, Result & result, Transmitter * transmitter,
                                     Receiver * receiver) ;
    static void CalculateReflections(const Record & record, Result & result, Transmitter * transmitter,
                                     Receiver * receiver);
    static void CalculateReflection(const glm::vec3 & tx_position, const glm::vec3 & rx_position,
                                    const float & tx_freq, const float & tx_gain,
                                    const float & rx_gain, const float & tx_power,
                                    const glm::vec3 & ref_position, Result & result);

    static void CalculateDiffraction(const Record & record, Result & result,
                                     Transmitter * transmitter, Receiver * receiver);

    static void GetDrawComponents( const glm::vec3 & start_position, const glm::vec3 &end_position,
    ...
    bool CalculatePathLoss(Transmitter* transmitter, Receiver * receiver,
                           ...
                           Result& result) const;
    static bool CalculatePathLossMap(glm::vec3 tx_position, float tx_frequency,
